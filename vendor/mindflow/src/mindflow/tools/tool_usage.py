...
from mindflow.tools.tool_calling import InstructorToolCalling, ToolCalling
...
class ToolUsage:
    ...
    async def _ause(
        ...
        if self.agent:
            ...
            mindflow_event_bus.emit(self, ToolUsageStartedEvent(**event_data))
    ...
    def on_tool_use_finished(
        ...
        if self.task:
            event_data["task_id"] = str(self.task.id)
            event_data["task_name"] = self.task.name or self.task.description
        mindflow_event_bus.emit(self, ToolUsageFinishedEvent(**event_data))
