...
from mindflow.flow.constants import AND_CONDITION, OR_CONDITION
from mindflow.flow.flow_wrappers import (
    FlowCondition,
    FlowConditions,
    FlowMethod,
    ListenMethod,
...
from mindflow.flow.types import FlowExecutionData, FlowMethodName, PendingListenerKey
from mindflow.flow.utils import (
...
if TYPE_CHECKING:
    from mindflow.flow.async_feedback.types import PendingFeedbackContext
    from mindflow.flow.human_feedback import HumanFeedbackResult
    from mindflow.llms.base_llm import BaseLLM

from mindflow.flow.visualization import build_flow_structure, render_interactive
from mindflow.types.streaming import CrewStreamingOutput, FlowStreamingOutput
...
class Flow(Generic[T], metaclass=FlowMeta):
    ...
    def from_pending(
        ...
        **kwargs: Any,
        ...
        if persistence is None:
            from mindflow.flow.persistence import SQLiteFlowPersistence
    ...
    async def resume_async(self, feedback: str = "") -> Any:
        ...
        try:
            ...
            from mindflow.flow.async_feedback.types import HumanFeedbackPending
    ...
    async def kickoff_async(
        ...
        try:
            ...
            if not self.suppress_flow_events:
                future = mindflow_event_bus.emit(
                    self,
                    FlowStartedEvent(
                        type="flow_started",
                        flow_name=self.name or self.__class__.__name__,
    ...
    async def _execute_start_method(self, start_method_name: FlowMethodName) -> None:
        ...
        Note:
            ...
            - Automatically injects mindflow_trigger_payload if available in flow inputs
        """
    ...
    def _inject_trigger_payload_for_start_method(
        ...
        def prepare_kwargs(
            *args: Any, **kwargs: Any
            ...
            trigger_payload = inputs.get("mindflow_trigger_payload")

            sig = inspect.signature(original_method)
            accepts_trigger_payload = "mindflow_trigger_payload" in sig.parameters

            if trigger_payload is not None and accepts_trigger_payload:
                kwargs["mindflow_trigger_payload"] = trigger_payload
            elif trigger_payload is not None:
                self._log_flow_event(
                    f"Trigger payload available but {original_method.__name__} doesn't accept mindflow_trigger_payload parameter"
                )
            return args, kwargs
    ...
    def _collapse_to_outcome(
        ...
        from mindflow.llms.base_llm import BaseLLM as BaseLLMClass
    ...
    def plot(self, filename: str = "mindflow_flow.html", show: bool = True) -> str:
        """Create interactive HTML visualization of Flow structure.

        Args:
            filename: Output HTML filename (default: "mindflow_flow.html").
