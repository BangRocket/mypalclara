...
from mindflow.types.streaming import (
    CrewStreamingOutput,
    FlowStreamingOutput,
    StreamChunk,
    StreamChunkType,
...
def _unregister_handler(handler: Callable[[Any, BaseEvent], None]) -> None:
    ...
    with mindflow_event_bus._rwlock.w_locked():
        handlers: frozenset[Callable[[Any, BaseEvent], None]] = (
            mindflow_event_bus._sync_handlers.get(LLMStreamChunkEvent, frozenset())
        )
        mindflow_event_bus._sync_handlers[LLMStreamChunkEvent] = handlers - {handler}
...
def create_streaming_state(
    ...
    mindflow_event_bus.register_handler(LLMStreamChunkEvent, handler)
