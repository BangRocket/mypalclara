...
from mindflow.agents.constants import FINAL_ANSWER_AND_PARSABLE_ACTION_ERROR_MESSAGE
from mindflow.agents.parser import (
    AgentAction,
    AgentFinish,
    OutputParserError,
    parse,
...
from mindflow.tools import BaseTool as MindflowTool
from mindflow.tools.base_tool import BaseTool
from mindflow.tools.structured_tool import CrewStructuredTool
from mindflow.tools.tool_types import ToolResult
...
from mindflow.utilities.printer import ColoredText, Printer
...
def parse_tools(tools: list[BaseTool]) -> list[CrewStructuredTool]:
    ...
    for tool in tools:
        if isinstance(tool, MindflowTool):
...
def handle_context_length(
    ...
    if respect_context_window:
        ...
        printer.print(
            content="Context length exceeded. Consider using smaller text or RAG tools from mindflow_tools.",
            color="red",
        )
        raise SystemExit(
            "Context length exceeded and user opted not to summarize. Consider using smaller text or RAG tools from mindflow_tools."
        )
...
def load_agent_from_repository(from_repository: str) -> dict[str, Any]:
    ...
    if from_repository:
        ...
        from mindflow.cli.authentication.token import get_auth_token
        ...
        if response.status_code == 404:
            raise AgentRepositoryError(
                ...
                f"\nIf you are using the wrong organization, switch to the correct one using `mindflow org switch <org_id>` command.",
            )
...
def _setup_before_llm_call_hooks(
    ...
    if executor_context and executor_context.before_llm_call_hooks:
        from mindflow.hooks.llm_hooks import LLMCallHookContext
