"""Base LLM abstract class for MindFlow.

This module provides the abstract base class for all LLM implementations
in MindFlow, including common functionality for native SDK implementations.
"""
...
class BaseLLM(ABC):
    ...
    def _emit_call_started_event(
        ...
        if not hasattr(mindflow_event_bus, "emit"):
            raise ValueError("mindflow_event_bus does not have an emit method") from None
    ...
    def _handle_tool_execution(
        ...
        try:
            ...
            if not hasattr(mindflow_event_bus, "emit"):
                raise ValueError(
                    "mindflow_event_bus does not have an emit method"
                ) from None
