...
from mindflow.cli.add_crew_to_flow import add_crew_to_flow
from mindflow.cli.authentication.main import AuthenticationCommand
from mindflow.cli.config import Settings
from mindflow.cli.create_crew import create_crew
from mindflow.cli.create_flow import create_flow
from mindflow.cli.crew_chat import run_chat
from mindflow.cli.deploy.main import DeployCommand
from mindflow.cli.enterprise.main import EnterpriseConfigureCommand
from mindflow.cli.evaluate_crew import evaluate_crew
from mindflow.cli.install_crew import install_crew
from mindflow.cli.kickoff_flow import kickoff_flow
from mindflow.cli.organization.main import OrganizationCommand
from mindflow.cli.plot_flow import plot_flow
from mindflow.cli.replay_from_task import replay_task_command
from mindflow.cli.reset_memories_command import reset_memories_command
from mindflow.cli.run_crew import run_crew
from mindflow.cli.settings.main import SettingsCommand
from mindflow.cli.tools.main import ToolCommand
from mindflow.cli.train_crew import train_crew
from mindflow.cli.triggers.main import TriggersCommand
from mindflow.cli.update_crew import update_crew
from mindflow.cli.utils import build_env_with_tool_repository_credentials, read_toml
from mindflow.memory.storage.kickoff_task_outputs_storage import (
    KickoffTaskOutputsSQLiteStorage,
)


@click.group()
@click.version_option(get_version("mindflow"))
def mindflow():
    """Top-level command group for mindflow."""
...
@mindflow.command()
@click.option(
    "--tools", is_flag=True, help="Show the installed version of mindflow tools"
)
def version(tools):
    """Show the installed version of mindflow."""
    try:
        mindflow_version = get_version("mindflow")
    except Exception:
        mindflow_version = "unknown version"
    click.echo(f"mindflow version: {mindflow_version}")

    if tools:
        try:
            tools_version = get_version("mindflow")
            click.echo(f"mindflow tools version: {tools_version}")
        except Exception:
            click.echo("mindflow tools not installed")
...
@mindflow.command()
def login():
    """Sign Up/Login to MindFlow Cloud."""
    Settings().clear_user_settings()
    AuthenticationCommand().login()


# DEPLOY MINDFLOW+ COMMANDS
@mindflow.group()
def deploy():
...
@mindflow.group()
def triggers():
    """Trigger related commands. Use 'mindflow triggers list' to see available triggers, or 'mindflow triggers run app_slug/trigger_slug' to execute."""
...
@mindflow.group(invoke_without_command=True)
def org():
...
@org.command()
def current():
    """Show current organization when 'mindflow org' is called without subcommands."""
    org_command = OrganizationCommand()
    org_command.current()
...
def enterprise_configure(enterprise_url: str):
    """Configure MindFlow Cloud OAuth2 settings from the provided Enterprise URL."""
...
def env_view():
    ...
    # Check MINDFLOW_TRACING_ENABLED
    mindflow_tracing = os.getenv("MINDFLOW_TRACING_ENABLED", "")
    if mindflow_tracing:
        table.add_row(
            "MINDFLOW_TRACING_ENABLED",
            mindflow_tracing,
            "Environment/Shell",
        )
    else:
    ...
    mindflow_testing = os.getenv("MINDFLOW_TESTING", "")
    if mindflow_testing:
        table.add_row("MINDFLOW_TESTING", mindflow_testing, "Environment/Shell")

    mindflow_user_id = os.getenv("MINDFLOW_USER_ID", "")
    if mindflow_user_id:
        table.add_row("MINDFLOW_USER_ID", mindflow_user_id, "Environment/Shell")

    mindflow_org_id = os.getenv("MINDFLOW_ORG_ID", "")
    if mindflow_org_id:
        table.add_row("MINDFLOW_ORG_ID", mindflow_org_id, "Environment/Shell")
    ...
    if env_file_exists:
        console.print(
            "\n[dim]💡 Tip: To enable tracing via .env, add: MINDFLOW_TRACING_ENABLED=true[/dim]"
        )
    else:
        console.print(
            "\n[dim]💡 Tip: Create a .env file in your project root and add: MINDFLOW_TRACING_ENABLED=true[/dim]"
        )
    console.print()
...
def traces_enable():
    ...
    from mindflow.events.listeners.tracing.utils import (
        _load_user_data,
        _save_user_data,
    )
    ...
    panel = Panel(
        ...
        "Your crew/flow executions will now send traces to MindFlow+.\n"
        "Use 'mindflow traces disable' to turn off trace collection.",
...
def traces_disable():
    ...
    panel = Panel(
        ...
        "Use 'mindflow traces enable' to turn trace collection back on.",
...
def traces_status():
    ...
    env_enabled = os.getenv("MINDFLOW_TRACING_ENABLED", "false")
    table.add_row("MINDFLOW_TRACING_ENABLED", env_enabled)
