...
from mindflow.cli.constants import (
    DEFAULT_MINDFLOW_CLOUD_URL,
    MINDFLOW_CLOUD_OAUTH2_AUDIENCE,
    MINDFLOW_CLOUD_OAUTH2_CLIENT_ID,
    MINDFLOW_CLOUD_OAUTH2_DOMAIN,
    MINDFLOW_CLOUD_OAUTH2_PROVIDER,
)
from mindflow.cli.shared.token_manager import TokenManager


logger = getLogger(__name__)

DEFAULT_CONFIG_PATH = Path.home() / ".config" / "mindflow" / "settings.json"
...
def get_writable_config_path() -> Path | None:
    ...
    Tries in order:
    1. Default: ~/.config/mindflow/settings.json
    2. Temp directory: /tmp/mindflow_settings.json (or OS equivalent)
    3. Current directory: ./mindflow_settings.json
    ...
    fallback_paths = [
        ...
        Path(tempfile.gettempdir()) / "mindflow_settings.json",  # Temporary directory
        Path.cwd() / "mindflow_settings.json",  # Current working directory
    ]

    for config_path in fallback_paths:
        try:
            config_path.parent.mkdir(parents=True, exist_ok=True)
            test_file = config_path.parent / ".mindflow_write_test"
            try:
...
DEFAULT_CLI_SETTINGS = {
    "enterprise_base_url": DEFAULT_MINDFLOW_CLOUD_URL,
    "oauth2_provider": MINDFLOW_CLOUD_OAUTH2_PROVIDER,
    "oauth2_audience": MINDFLOW_CLOUD_OAUTH2_AUDIENCE,
    "oauth2_client_id": MINDFLOW_CLOUD_OAUTH2_CLIENT_ID,
    "oauth2_domain": MINDFLOW_CLOUD_OAUTH2_DOMAIN,
    "oauth2_extra": {},
}
...
class Settings(BaseModel):
    enterprise_base_url: str | None = Field(
        ...
        description="Base URL of the MindFlow Cloud instance",
    )
