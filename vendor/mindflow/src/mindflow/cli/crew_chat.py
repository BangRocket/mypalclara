...
from mindflow.cli.utils import read_toml
...
from mindflow.types.crew_chat import ChatInputField, ChatInputs
...
def check_conversational_crews_version(
    mindflow_version: str, pyproject_data: dict[str, Any]
) -> bool:
    """
    Check if the installed MindFlow version supports conversational crews.

    Args:
        mindflow_version: The current version of MindFlow.
    ...
    """
    try:
        if version.parse(mindflow_version) < version.parse(MIN_REQUIRED_VERSION):
            click.secho(
                "You are using an older version of MindFlow that doesn't support conversational crews. "
                "Run 'uv upgrade mindflow' to get the latest version.",
                fg="red",
            )
            return False
    except version.InvalidVersion:
        click.secho("Invalid MindFlow version format detected.", fg="red")
        return False
    return True
...
def run_chat() -> None:
    ...
    if not check_conversational_crews_version(mindflow_version, pyproject_data):
        return
...
def build_system_message(crew_chat_inputs: ChatInputs) -> str:
    ...
    return (
        "You are a helpful AI assistant for the MindFlow platform. "
