...
from mindflow.cli.command import BaseCommand
from mindflow.cli.config import HIDDEN_SETTINGS_KEYS, READONLY_SETTINGS_KEYS, Settings
from mindflow.events.listeners.tracing.utils import _load_user_data
...
class SettingsCommand(BaseCommand):
    ...
    def list(self) -> None:
        ...
        table = Table(title="MindFlow CLI Configuration")
        ...
        # MINDFLOW_TRACING_ENABLED environment variable
        env_tracing = os.getenv("MINDFLOW_TRACING_ENABLED") or os.getenv("CREWAI_TRACING_ENABLED", "")
    ...
    def reset_all_settings(self) -> None:
        ...
        self.settings.reset()
        console.print(
            "Successfully reset all configuration parameters to default values. It is recommended to run [bold yellow]'mindflow login'[/bold yellow] to re-authenticate.",
            style="bold green",
        )
