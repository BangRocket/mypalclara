...
from mindflow.cli import git
from mindflow.cli.command import BaseCommand, PlusAPIMixin
from mindflow.cli.config import Settings
from mindflow.cli.constants import DEFAULT_MINDFLOW_CLOUD_URL
from mindflow.cli.utils import (
...
class ToolCommand(BaseCommand, PlusAPIMixin):
    """
    A class to handle tool repository related operations for MindFlow projects.
    """
    ...
    def login(self) -> None:
        ...
        if login_response.status_code != 200:
            console.print(
                "Authentication failed. Verify if the currently active organization can access the tool repository, and run 'mindflow login' again.",
                style="bold red",
            )
            try:
                console.print(
