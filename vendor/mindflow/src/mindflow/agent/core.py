...
from mindflow.a2a.config import A2AConfig
from mindflow.agent.utils import (
...
from mindflow.agents.crew_agent_executor import CrewAgentExecutor
...
from mindflow.experimental.crew_agent_executor_flow import CrewAgentExecutorFlow
...
from mindflow.lite_agent import LiteAgent
from mindflow.llms.base_llm import BaseLLM
from mindflow.mcp import (
    MCPClient,
    MCPServerConfig,
    MCPServerHTTP,
    MCPServerSSE,
...
from mindflow.memory.contextual.contextual_memory import ContextualMemory
...
from mindflow.utilities.agent_utils import (
    get_tool_names,
    load_agent_from_repository,
    parse_tools,
    render_text_description_and_args,
)
from mindflow.utilities.constants import TRAINED_AGENTS_DATA_FILE, TRAINING_DATA_FILE
from mindflow.utilities.converter import Converter
from mindflow.utilities.guardrail_types import GuardrailType
from mindflow.utilities.llm_utils import create_llm
from mindflow.utilities.prompts import Prompts
from mindflow.utilities.token_counter_callback import TokenCalcHandler
from mindflow.utilities.training_handler import CrewTrainingHandler


if TYPE_CHECKING:
    from mindflow_tools import CodeInterpreterTool

    from mindflow.agents.agent_builder.base_agent import PlatformAppOrAction
    ...
    from mindflow.utilities.types import LLMMessage
...
class Agent(BaseAgent):
    ...
    Attributes:
            ...
            apps: List of applications that the agent can access through MindFlow Platform.
    ...
    def get_platform_tools(self, apps: list[PlatformAppOrAction]) -> list[BaseTool]:
        try:
            from mindflow_tools import (
                MindflowPlatformTools,
            )

            return MindflowPlatformTools(apps=apps)
        except Exception as e:
    ...
    def get_mcp_tools(self, mcps: list[str | MCPServerConfig]) -> list[BaseTool]:
        """Convert MCP server references/configs to MindFlow tools.
    ...
    def _get_mcp_tools_from_string(self, mcp_ref: str) -> list[BaseTool]:
        ...
        MCP references (https://... and mindflow-cloud:...).
        ...
        """
        if mcp_ref.startswith("mindflow-cloud:"):
    ...
    def _get_external_mcp_tools(self, mcp_ref: str) -> list[BaseTool]:
        ...
        from mindflow.tools.mcp_tool_wrapper import MCPToolWrapper
    ...
    def _get_native_mcp_tools(
        ...
        """
        from mindflow.tools.base_tool import BaseTool
        from mindflow.tools.mcp_native_tool import MCPNativeTool
            ...
            if mcp_config.tool_filter:
                ...
                for tool in tools_list:
                    if callable(mcp_config.tool_filter):
                        try:
                            from mindflow.mcp.filters import ToolFilterContext
    ...
    def _get_amp_mcp_tools(self, amp_ref: str) -> list[BaseTool]:
        """Get tools from MindFlow Cloud MCP marketplace."""
        # Parse: "mindflow-cloud:mcp-name" or "mindflow-cloud:mcp-name#tool_name"
        amp_part = amp_ref.replace("mindflow-cloud:", "")
        if "#" in amp_part:
    ...
    def _fetch_amp_mcp_servers(mcp_name: str) -> list[dict[str, Any]]:
        """Fetch MCP server configurations from MindFlow Cloud API."""
    ...
    def get_multimodal_tools() -> Sequence[BaseTool]:
        from mindflow.tools.agent_tools.add_image_tool import AddImageTool
    ...
    def get_code_execution_tools(self) -> list[CodeInterpreterTool]:
        try:
            ...
            self._logger.log(
                "info", "Coding tools not available. Install mindflow_tools. "
            )
            return []
