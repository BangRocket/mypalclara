...
from mindflow.security import Fingerprint, SecurityConfig
...
from mindflow.utilities.config import process_config
from mindflow.utilities.constants import NOT_SPECIFIED, _NotSpecified
from mindflow.utilities.converter import Converter, convert_to_model
from mindflow.utilities.guardrail import (
    process_guardrail,
)
from mindflow.utilities.guardrail_types import (
    GuardrailCallable,
    GuardrailType,
    GuardrailsType,
)
from mindflow.utilities.i18n import I18N, get_i18n
from mindflow.utilities.printer import Printer
from mindflow.utilities.string_utils import interpolate_only
...
class Task(BaseModel):
    ...
    Attributes:
        ...
        allow_mindflow_trigger_context: Optional flag to control mindflow_trigger_payload injection.
    ...
    allow_mindflow_trigger_context: bool | None = Field(
        default=None,
        description="Whether this task should append 'Trigger Payload: {mindflow_trigger_payload}' to the task description when mindflow_trigger_payload exists in crew inputs.",
    )
    ...
    def handle_max_retries_deprecation(self) -> Self:
        if self.max_retries is not None:
            warnings.warn(
                "The 'max_retries' parameter is deprecated and will be removed in MindFlow v1.0.0. "
    ...
    async def _aexecute_core(
        ...
        try:
            ...
            mindflow_event_bus.emit(self, TaskStartedEvent(context=context, task=self))  # type: ignore[no-untyped-call]
            ...
            self.end_time = datetime.datetime.now()
            mindflow_event_bus.emit(self, TaskFailedEvent(error=str(e), task=self))  # type: ignore[no-untyped-call]
...
- Use ```language for code blocks"""
    ...
    def _save_file(self, result: dict[str, Any] | str | Any) -> None:
        ...
        Note:
            ...
            from the mindflow_tools package:
                pip install 'mindflow[tools]'
                from mindflow_tools import FileWriterTool
        ...
        Raises:
            ...
            RuntimeError: If there is an error writing to the file. For cross-platform
                compatibility, especially on Windows, use FileWriterTool from mindflow_tools
                package.
        """
        ...
        filewriter_recommendation = (
            ...
            "use FileWriterTool from mindflow_tools package."
        )
