...
from mindflow.llms.constants import (
    ANTHROPIC_MODELS,
    AZURE_MODELS,
    BEDROCK_MODELS,
    GEMINI_MODELS,
...
from mindflow.utilities import InternalInstructor
from mindflow.utilities.exceptions.context_window_exceeding_exception import (
    LLMContextLengthExceededError,
)
from mindflow.utilities.logger_utils import suppress_warnings
...
if TYPE_CHECKING:
    ...
    from mindflow.llms.providers.anthropic.completion import AnthropicThinkingConfig
...
class LLM(BaseLLM):
    ...
    def _get_native_provider(cls, provider: str) -> type | None:
        ...
        if provider == "openai":
            from mindflow.llms.providers.openai.completion import OpenAICompletion

            return OpenAICompletion

        if provider == "anthropic" or provider == "claude":
            from mindflow.llms.providers.anthropic.completion import (
                AnthropicCompletion,
            )
        ...
        if provider == "azure" or provider == "azure_openai":
            from mindflow.llms.providers.azure.completion import AzureCompletion

            return AzureCompletion

        if provider == "google" or provider == "gemini":
            from mindflow.llms.providers.gemini.completion import GeminiCompletion

            return GeminiCompletion

        if provider == "bedrock":
            from mindflow.llms.providers.bedrock.completion import BedrockCompletion
