...
from mindflow.agent.internal.meta import AgentMeta
from mindflow.agents.agent_builder.utilities.base_token_process import TokenProcess
...
class BaseAgent(BaseModel, ABC, metaclass=AgentMeta):
    """Abstract Base Class for all third party agents compatible with MindFlow.
    ...
    Attributes:
        ...
        apps: List of enterprise applications that the agent can access through MindFlow Cloud Tools.
    ...
    apps: list[PlatformAppOrAction] | None = Field(
        default=None,
        description="List of applications or application/action combinations that the agent can access through MindFlow Platform. Can contain app names (e.g., 'gmail') or specific actions (e.g., 'gmail/send_email')",
    )
    mcps: list[str | MCPServerConfig] | None = Field(
        default=None,
        description="List of MCP server references. Supports 'https://server.com/path' for external servers and 'mindflow-cloud:mcp-name' for AMP marketplace. Use '#tool_name' suffix for specific tools.",
    )
    ...
    def validate_mcps(
        ...
        for mcp in mcps:
            if isinstance(mcp, str):
                if mcp.startswith(("https://", "mindflow-cloud:")):
                    validated_mcps.append(mcp)
                else:
                    raise ValueError(
                        f"Invalid MCP reference: {mcp}. "
                        "String references must start with 'https://' or 'mindflow-cloud:'"
                    )
