"""Event bus for managing and dispatching events in MindFlow.

This module provides a singleton event bus that allows registration and handling
of events throughout the MindFlow system, supporting both synchronous and asynchronous
...
from mindflow.events.handler_graph import build_execution_plan
from mindflow.events.types.event_bus_types import (
    AsyncHandler,
    AsyncHandlerSet,
    ExecutionPlan,
    Handler,
...
from mindflow.events.types.llm_events import LLMStreamChunkEvent
from mindflow.events.utils.console_formatter import ConsoleFormatter
from mindflow.events.utils.handlers import is_async_handler, is_call_handler_safe
from mindflow.utilities.rw_lock import RWLock
...
class MindflowEventsBus:
    """Singleton event bus for handling events in MindFlow.
    ...
    def __new__(cls) -> Self:
        ...
        Returns:
            The singleton MindflowEventsBus instance
        """
    ...
    def _initialize(self) -> None:
        ...
        self._sync_executor = ThreadPoolExecutor(
            max_workers=10,
            thread_name_prefix="MindflowSyncHandler",
        )
    ...
    def on(
        ...
        Example:
            >>> from mindflow.events import mindflow_event_bus, Depends
            >>> from mindflow.events.types.llm_events import LLMCallStartedEvent
            >>>
            >>> @mindflow_event_bus.on(LLMCallStartedEvent)
            >>> def setup_context(source, event):
            ...     print("Setting up context")
            >>>
            >>> @mindflow_event_bus.on(LLMCallStartedEvent, depends_on=Depends(setup_context))
    ...
    def _call_handlers(
        ...
        if errors:
            for handler, error in errors:
                self._console.print(
                    f"[MindflowEventsBus] Sync handler error in {handler.__name__}: {error}"
                )
    ...
    async def _acall_handlers(
        ...
        for handler, result in zip(handlers, results, strict=False):
            if isinstance(result, Exception):
                self._console.print(
                    f"[MindflowEventsBus] Async handler error in {getattr(handler, '__name__', handler)}: {result}"
                )
    ...
    def emit(self, source: Any, event: BaseEvent) -> Future[None] | None:
        ...
        Example:
            >>> future = mindflow_event_bus.emit(source, event)
            >>> if future:
        ...
        with self._rwlock.r_locked():
            if self._shutting_down:
                self._console.print(
                    "[MindflowEventsBus] Warning: Attempted to emit event during shutdown. Ignoring."
                )
                return None
    ...
    def scoped_handlers(self) -> Generator[None, Any, None]:
        ...
        Example:
            ...
            >>> from mindflow.events.event_types import CrewKickoffStartedEvent
    ...
    def shutdown(self, wait: bool = True) -> None:
        ...
        if wait:
            ...
            try:
                ...
                self._console.print(f"[MindflowEventsBus] Error waiting for tasks: {e}")
        else:
...
mindflow_event_bus: Final[MindflowEventsBus] = MindflowEventsBus()

atexit.register(mindflow_event_bus.shutdown)
