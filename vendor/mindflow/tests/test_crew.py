...
from mindflow.agents import CacheHandler
...
from mindflow.flow import Flow, start
...
from mindflow.project import CrewBase, agent, before_kickoff, crew, task
from mindflow.task import Task
from mindflow.tasks.conditional_task import ConditionalTask
from mindflow.tasks.output_format import OutputFormat
from mindflow.tasks.task_output import TaskOutput
from mindflow.tools import BaseTool, tool
...
def test_warning_long_term_memory_without_entity_memory():
    ...
    with (
        patch("mindflow.utilities.printer.Printer.print") as mock_print,
        patch(
            "mindflow.memory.long_term.long_term_memory.LongTermMemory.save"
        ) as save_memory,
    ):
        crew.kickoff()
...
def test_long_term_memory_with_memory_flag():
    ...
    with (
        patch("mindflow.utilities.printer.Printer.print") as mock_print,
        patch("mindflow.memory.long_term.long_term_memory.LongTermMemory.save") as save_memory,
    ):
        crew = Crew(
            agents=[math_researcher],
            tasks=[task1],
...
def test_replay_with_context():
    ...
    with patch(
        "mindflow.utilities.task_output_storage_handler.TaskOutputStorageHandler.load",
        return_value=[
            {
...
def test_replay_with_context_set_to_nullable():
    ...
    with patch("mindflow.task.Task.execute_sync") as mock_execute_task:
        mock_execute_task.return_value = TaskOutput(
            description="Test Task Output",
            raw="test raw output",
            agent="test_agent",
