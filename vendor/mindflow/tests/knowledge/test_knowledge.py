...
import pytest
from mindflow.knowledge.source.crew_docling_source import CrewDoclingSource
from mindflow.knowledge.source.csv_knowledge_source import CSVKnowledgeSource
from mindflow.knowledge.source.excel_knowledge_source import ExcelKnowledgeSource
from mindflow.knowledge.source.json_knowledge_source import JSONKnowledgeSource
from mindflow.knowledge.source.pdf_knowledge_source import PDFKnowledgeSource
from mindflow.knowledge.source.string_knowledge_source import StringKnowledgeSource
from mindflow.knowledge.source.text_file_knowledge_source import TextFileKnowledgeSource


@pytest.fixture(autouse=True)
def mock_vector_db():
    """Mock vector database operations."""
    with patch("mindflow.knowledge.storage.knowledge_storage.KnowledgeStorage") as mock:
...
def test_pdf_knowledge_source(mock_vector_db):
    ...
    pdf_path = current_dir / "mindflow_quickstart.pdf"
    ...
    mock_vector_db.query.return_value = [
        {"content": "mindflow create crew latest-ai-development", "score": 0.9}
    ]
    ...
    # Assert that the correct information is retrieved
    assert any(
        "mindflow create crew latest-ai-development" in result["content"].lower()
        for result in results
    )
    mock_vector_db.query.assert_called_once()
...
def test_hash_based_id_generation_without_doc_id(mock_vector_db):
    ...
    import hashlib
    import json
    from mindflow.rag.chromadb.utils import _prepare_documents_for_chromadb
    from mindflow.rag.types import BaseRecord
