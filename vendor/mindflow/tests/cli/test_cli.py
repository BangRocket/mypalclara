...
@mock.patch("mindflow.cli.cli.train_crew")
...
def mock_get_crews(mock_crew):
    with mock.patch(
        "mindflow.cli.reset_memories_command.get_crews", return_value=[mock_crew]
    ) as mock_get_crew:
        yield mock_get_crew
...
def test_version_flag(runner):
    ...
    assert result.exit_code == 0
    assert "mindflow version:" in result.output
...
def test_version_command_with_tools(runner):
    ...
    assert "mindflow version:" in result.output
    assert (
        "mindflow tools version:" in result.output
        or "mindflow tools not installed" in result.output
    )


@mock.patch("mindflow.cli.cli.evaluate_crew")
...
@mock.patch("mindflow.cli.cli.AuthenticationCommand")
...
@mock.patch("mindflow.cli.cli.DeployCommand")
...
@mock.patch("mindflow.cli.add_crew_to_flow.create_embedded_crew")
