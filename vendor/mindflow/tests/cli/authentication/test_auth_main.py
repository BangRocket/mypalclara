...
from mindflow.cli.constants import (
    MINDFLOW_CLOUD_OAUTH2_AUDIENCE,
    MINDFLOW_CLOUD_OAUTH2_CLIENT_ID,
    MINDFLOW_CLOUD_OAUTH2_DOMAIN,
)
...
class TestAuthenticationCommand:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "workos",
                {
                    "device_code_url": f"https://{MINDFLOW_CLOUD_OAUTH2_DOMAIN}/oauth2/device_authorization",
                    "token_url": f"https://{MINDFLOW_CLOUD_OAUTH2_DOMAIN}/oauth2/token",
                    "client_id": MINDFLOW_CLOUD_OAUTH2_CLIENT_ID,
                    "audience": MINDFLOW_CLOUD_OAUTH2_AUDIENCE,
                    "domain": MINDFLOW_CLOUD_OAUTH2_DOMAIN,
                },
            ),
        ],
    )
    @patch("mindflow.cli.authentication.main.AuthenticationCommand._get_device_code")
    @patch(
        "mindflow.cli.authentication.main.AuthenticationCommand._display_auth_instructions"
    )
    @patch("mindflow.cli.authentication.main.AuthenticationCommand._poll_for_token")
    @patch("mindflow.cli.authentication.main.console.print")
    def test_login(
        self,
        mock_console_print,
        mock_poll,
    ...
    @patch("mindflow.cli.authentication.main.webbrowser")
    @patch("mindflow.cli.authentication.main.console.print")
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "workos",
                {
                    "jwks_url": f"https://{MINDFLOW_CLOUD_OAUTH2_DOMAIN}/oauth2/jwks",
                    "issuer": f"https://{MINDFLOW_CLOUD_OAUTH2_DOMAIN}",
                    "audience": MINDFLOW_CLOUD_OAUTH2_AUDIENCE,
                },
            ),
        ],
    ...
    @patch("mindflow.cli.authentication.main.validate_jwt_token")
    @patch("mindflow.cli.authentication.main.TokenManager.save_tokens")
    def test_validate_and_save_token(
        ...
        from mindflow.cli.authentication.main import Oauth2Settings
        from mindflow.cli.authentication.providers.workos import WorkosProvider
    ...
    @patch("mindflow.cli.tools.main.ToolCommand")
    @patch("mindflow.cli.authentication.main.Settings")
    @patch("mindflow.cli.authentication.main.console.print")
    ...
    def test_poll_for_token_success(self, mock_console_print, mock_post):
        ...
        with (
            ...
            expected_calls = [
                ...
                call("\n[bold green]Welcome to MindFlow Cloud![/bold green]\n"),
            ]
            mock_console_print.assert_has_calls(expected_calls)
