...
def test_interpolate_only():
    ...
    result = interpolate_only(
        input_string=normal_string, inputs={"name": "John", "place": "MindFlow"}
    )
    assert result == "Hello John, welcome to MindFlow!"
...
def test_task_copy_with_not_specified_context():
    from mindflow.utilities.constants import NOT_SPECIFIED
    original_task = Task(
        description="Test task",
        expected_output="Test output",
    )
