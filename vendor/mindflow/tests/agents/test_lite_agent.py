...
from mindflow.events.types.agent_events import LiteAgentExecutionStartedEvent
from mindflow.events.types.tool_usage_events import ToolUsageStartedEvent
...
from mindflow import LLM, Agent
...
def test_lite_agent_created_with_correct_parameters(monkeypatch, verbose):
    ...
    monkeypatch.setattr("mindflow.agent.core.LiteAgent", MockLiteAgent)
...
def test_guardrail_is_called_using_string():
    ...
    from mindflow.events.event_types import (
...
def test_lite_agent_with_invalid_llm():
    ...
    with patch("mindflow.lite_agent.create_llm", return_value=None):
...
@patch.dict("os.environ", {"MINDFLOW_PLATFORM_INTEGRATION_TOKEN": "test_token"})
@patch("mindflow_tools.tools.mindflow_platform_tools.mindflow_platform_tool_builder.requests.get")
@pytest.mark.vcr()
...
@patch("mindflow.agent.Agent._get_external_mcp_tools")
@pytest.mark.vcr()
