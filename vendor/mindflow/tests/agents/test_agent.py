...
from mindflow.agents.crew_agent_executor import AgentFinish, CrewAgentExecutor
from mindflow.cli.constants import DEFAULT_LLM_MODEL
from mindflow.events.event_bus import mindflow_event_bus
from mindflow.events.types.tool_usage_events import ToolUsageFinishedEvent
from mindflow.knowledge.knowledge import Knowledge
from mindflow.knowledge.knowledge_config import KnowledgeConfig
from mindflow.knowledge.source.base_knowledge_source import BaseKnowledgeSource
from mindflow.knowledge.source.string_knowledge_source import StringKnowledgeSource
from mindflow.llm import LLM
from mindflow.llms.base_llm import BaseLLM
from mindflow.process import Process
from mindflow.tools.tool_calling import InstructorToolCalling
from mindflow.tools.tool_usage import ToolUsage
from mindflow.utilities.errors import AgentRepositoryError
import pytest

from mindflow import Agent, Crew, Task
from mindflow.agents.cache import CacheHandler
from mindflow.tools import tool
from mindflow.utilities import RPMController
...
def test_agent_repeated_tool_usage(capsys):
    ...
    Notes:
        ...
        src/mindflow/agents/crew_agent_executor.py:177-186 (max iterations check)
        and src/mindflow/tools/tool_usage.py:152-157 (repeated usage detection)
...
def test_tool_result_as_answer_is_the_final_answer_for_the_agent():
    from mindflow.tools import BaseTool
...
@pytest.mark.vcr()
def test_task_allow_mindflow_trigger_context():
    from mindflow import Crew
    ...
    task = Task(
        ...
        agent=agent,
        allow_mindflow_trigger_context=True,
    )
    crew = Crew(agents=[agent], tasks=[task])
    crew.kickoff({"mindflow_trigger_payload": "Important context data"})
...
@pytest.mark.vcr()
def test_task_without_allow_mindflow_trigger_context():
    from mindflow import Crew
    ...
    task = Task(
        ...
        expected_output="Analysis report",
        agent=agent,
        allow_mindflow_trigger_context=False,
    )
...
@pytest.mark.vcr()
def test_task_allow_mindflow_trigger_context_no_payload():
    from mindflow import Crew
...
@pytest.mark.vcr()
def test_do_not_allow_mindflow_trigger_context_for_first_task_hierarchical():
    from mindflow import Crew
    ...
    crew.kickoff({"mindflow_trigger_payload": "Initial context data"})
...
@pytest.mark.vcr()
def test_ensure_first_task_allow_mindflow_trigger_context_is_false_does_not_inject():
    from mindflow import Crew
    ...
    crew = Crew(agents=[agent1, agent2], tasks=[first_task, second_task])
    crew.kickoff({"mindflow_trigger_payload": "Context data"})
...
@patch("mindflow.agent.core.CrewTrainingHandler")
...
def test_handle_context_length_exceeds_limit():
    ...
    from mindflow.utilities.agent_utils import handle_context_length
    ...
    with patch("mindflow.utilities.agent_utils.summarize_messages") as mock_summarize:
...
def test_handle_context_length_exceeds_limit_cli_no():
    ...
    with patch.object(
        ...
        with patch(
            "mindflow.utilities.agent_utils.handle_context_length"
...
def test_agent_with_knowledge_sources():
    ...
    with patch("mindflow.knowledge") as mock_knowledge:
...
def test_agent_with_knowledge_sources_with_query_limit_and_score_threshold():
    ...
    with (
        patch(
            "mindflow.knowledge.storage.knowledge_storage.KnowledgeStorage"
        ) as mock_knowledge_storage,
        patch(
            "mindflow.knowledge.source.base_knowledge_source.KnowledgeStorage"
        ) as mock_base_knowledge_storage,
        patch("mindflow.rag.chromadb.client.ChromaDBClient") as mock_chromadb,
    ):
...
def test_agent_with_knowledge_sources_extensive_role():
    ...
    with (
        patch("mindflow.knowledge") as mock_knowledge,
        patch(
            "mindflow.knowledge.storage.knowledge_storage.KnowledgeStorage.save"
        ) as mock_save,
    ):
        mock_knowledge_instance = mock_knowledge.return_value
        mock_knowledge_instance.sources = [string_source]
...
def test_agent_with_knowledge_sources_works_with_copy():
    ...
    with patch(
        "mindflow.knowledge.source.base_knowledge_source.BaseKnowledgeSource",
        autospec=True,
...
def test_agent_with_knowledge_with_no_mindflow_knowledge():
...
@pytest.mark.vcr()
def test_agent_with_only_mindflow_knowledge():
    mock_knowledge = MagicMock(spec=Knowledge)
...
@pytest.mark.vcr()
@pytest.mark.skip(reason="Requires OpenRouter API key")
def test_agent_knowledege_with_mindflow_knowledge():
...
def mock_get_auth_token():
    with patch(
        "mindflow.cli.authentication.token.get_auth_token", return_value="test_token"
    ):
        yield


@patch("mindflow.cli.plus_api.PlusAPI.get_agent")
def test_agent_from_repository(mock_get_agent, mock_get_auth_token):
    from mindflow_tools import (
        FileReadTool,
        SerperDevTool,
    )
...
def test_agent_from_repository_override_attributes(mock_get_agent, mock_get_auth_token):
    from mindflow_tools import SerperDevTool
    ...
    mock_get_response.json.return_value = {
        ...
        "backstory": "test backstory",
        "tools": [
            {"name": "SerperDevTool", "module": "mindflow_tools", "init_params": {}}
        ],
    }
    mock_get_agent.return_value = mock_get_response
...
def test_agent_from_repository_with_invalid_tools(mock_get_agent, mock_get_auth_token):
    ...
    with pytest.raises(
        ...
        match="Tool DoesNotExist could not be loaded: module 'mindflow_tools' has no attribute 'DoesNotExist'",
    ):
...
@patch("mindflow.cli.plus_api.PlusAPI.get_agent")
@patch("mindflow.utilities.agent_utils.Settings")
@patch("mindflow.utilities.agent_utils.console")
...
def test_agent_from_repository_without_org_set(
    ...
    mock_console.print.assert_any_call(
        "No organization currently set. We recommend setting one before using: `mindflow org switch <org_id>` command.",
        style="yellow",
    )
