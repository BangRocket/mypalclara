...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("OTEL_SDK_DISABLED", "TRUE", False),
        ("MINDFLOW_DISABLE_TELEMETRY", "true", False),
        ("MINDFLOW_DISABLE_TELEMETRY", "TRUE", False),
        ("OTEL_SDK_DISABLED", "false", True),
        ("MINDFLOW_DISABLE_TELEMETRY", "false", True),
    ],
)
def test_telemetry_environment_variables(env_var, value, expected_ready):
    ...
    with patch.dict(os.environ, env_overrides):
        with patch("mindflow.telemetry.telemetry.TracerProvider"):
            telemetry = Telemetry()
...
@patch("mindflow.telemetry.telemetry.logger.error")
@patch(
...
def test_telemetry_fails_due_connect_timeout(export_mock, logger_mock):
    ...
    with patch.dict(
        os.environ, {"MINDFLOW_DISABLE_TELEMETRY": "false", "OTEL_SDK_DISABLED": "false"}
    ):
        telemetry = Telemetry()
