...
from mindflow.events.listeners.tracing.first_time_trace_handler import (
    FirstTimeTraceHandler,
)
from mindflow.events.listeners.tracing.trace_batch_manager import (
    TraceBatchManager,
)
from mindflow.events.listeners.tracing.trace_listener import (
    TraceCollectionListener,
)
from mindflow.events.listeners.tracing.types import TraceEvent
from mindflow.flow.flow import Flow, start
...
class TestTraceListenerSetup:
    ...
    def mock_user_data_file_io(self):
        ...
        with (
            patch(
                "mindflow.events.listeners.tracing.utils._load_user_data",
                return_value={},
            ),
            patch(
                "mindflow.events.listeners.tracing.utils._save_user_data",
                return_value=None,
            ),
        ):
            yield
    ...
    def mock_auth_token(self):
        ...
        with (
            patch(
                "mindflow.cli.authentication.token.get_auth_token",
                return_value="mock_token_12345",
            ),
            patch(
                "mindflow.events.listeners.tracing.trace_listener.get_auth_token",
                return_value="mock_token_12345",
            ),
            patch(
                "mindflow.events.listeners.tracing.trace_batch_manager.get_auth_token",
                return_value="mock_token_12345",
            ),
        ):
            yield
    ...
    def reset_tracing_singletons(self):
        ...
        from mindflow.events.listeners.tracing.utils import _tracing_enabled
        ...
        with mindflow_event_bus._rwlock.w_locked():
            mindflow_event_bus._sync_handlers = {}
            mindflow_event_bus._async_handlers = {}
            mindflow_event_bus._handler_dependencies = {}
            mindflow_event_bus._execution_plan_cache = {}
    ...
    def test_trace_listener_collects_crew_events(self):
        ...
        with patch.dict(
            os.environ,
            {
                "MINDFLOW_TRACING_ENABLED": "true",
                "MINDFLOW_DISABLE_TELEMETRY": "false",
                "MINDFLOW_DISABLE_TRACKING": "false",
    ...
    def test_batch_manager_finalizes_batch_clears_buffer(self):
        ...
        with patch.dict(
            ...
            trace_listener = None
            with mindflow_event_bus._rwlock.r_locked():
                for handler_set in mindflow_event_bus._sync_handlers.values():
                ...
                if not trace_listener:
                    for handler_set in mindflow_event_bus._async_handlers.values():
                        for handler in handler_set:
                            if hasattr(handler, "__self__") and isinstance(
                                handler.__self__, TraceCollectionListener
                            ):
    ...
    def test_trace_listener_disabled_when_env_false(self):
        ...
        with patch.dict(os.environ, {"MINDFLOW_TRACING_ENABLED": "false"}):
            agent = Agent(
                role="Test Agent",
                goal="Test goal",
                backstory="Test backstory",
            ...
            trace_handlers = []
            with mindflow_event_bus._rwlock.r_locked():
                for handlers in mindflow_event_bus._sync_handlers.values():
                    for handler in handlers:
                        if hasattr(handler, "__self__") and isinstance(
                            handler.__self__, TraceCollectionListener
                        ):
                            trace_handlers.append(handler)
                for handlers in mindflow_event_bus._async_handlers.values():
                    for handler in handlers:
                        if hasattr(handler, "__self__") and isinstance(
                            handler.__self__, TraceCollectionListener
                        ):
    ...
    def test_trace_listener_ephemeral_batch(self):
        ...
        with (
            ...
            patch(
                "mindflow.events.listeners.tracing.trace_listener.TraceCollectionListener._check_authenticated",
                return_value=False,
            ),
        ):
            agent = Agent(
    ...
    def test_first_time_user_trace_collection_with_timeout(self, mock_plus_api_calls):
        ...
        with (
            patch.dict(
                os.environ,
                {
                    "MINDFLOW_TRACING_ENABLED": "false",
                    "MINDFLOW_DISABLE_TELEMETRY": "false",
                    "MINDFLOW_DISABLE_TRACKING": "false",
                    "OTEL_SDK_DISABLED": "false",
                },
            ...
            patch(
                "mindflow.events.listeners.tracing.utils._is_test_environment",
                return_value=False,
            ),
            patch(
                "mindflow.events.listeners.tracing.utils.should_auto_collect_first_time_traces",
                return_value=True,
            ),
            patch(
                "mindflow.events.listeners.tracing.utils.is_first_execution",
                return_value=True,
            ),
            patch(
                "mindflow.events.listeners.tracing.first_time_trace_handler.prompt_user_for_trace_viewing",
                return_value=False,
            ) as mock_prompt,
            patch(
                "mindflow.events.listeners.tracing.first_time_trace_handler.mark_first_execution_completed"
    ...
    def test_first_time_user_trace_collection_user_accepts(self, mock_plus_api_calls):
        ...
        with (
            ...
            trace_listener.batch_manager.ephemeral_trace_url = (
                "https://mindflow.jorsh.app/trace/mock-id"
            )
    ...
    def test_trace_batch_marked_as_failed_on_finalize_error(self):
        ...
        with patch("mindflow.events.listeners.tracing.trace_batch_manager.is_tracing_enabled_in_context", return_value=True):
