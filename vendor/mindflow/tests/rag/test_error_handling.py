...
import pytest
from mindflow.knowledge.storage.knowledge_storage import (  # type: ignore[import-untyped]
    KnowledgeStorage,
)
from mindflow.memory.storage.rag_storage import RAGStorage  # type: ignore[import-untyped]


@patch("mindflow.knowledge.storage.knowledge_storage.get_rag_client")
...
def test_knowledge_storage_invalid_embedding_config(mock_get_client: MagicMock) -> None:
    ...
    with patch(
        "mindflow.knowledge.storage.knowledge_storage.build_embedder"
...
@patch("mindflow.memory.storage.rag_storage.get_rag_client")
...
def test_knowledge_storage_embedding_dimension_mismatch_detailed(
    ...
    assert "mindflow reset-memories -a" in str(exc_info.value)
