...
def test_internal_crew_with_mcp():
    from mindflow_tools.adapters.tool_collection import ToolCollection

    mock_adapter = Mock()
    mock_adapter.tools = ToolCollection([simple_tool, another_simple_tool])

    with (
        patch("mindflow_tools.MCPServerAdapter", return_value=mock_adapter) as adapter_mock,
        patch("mindflow.llm.LLM.__new__", return_value=Mock()),
    ):
