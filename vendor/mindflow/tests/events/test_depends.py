...
from mindflow.events import Depends, mindflow_event_bus
...
async def test_chain_of_dependencies():
    ...
    with mindflow_event_bus.scoped_handlers():
        ...
        @mindflow_event_bus.on(DependsTestEvent, depends_on=Depends(handler_a))
        def handler_b(source, event: DependsTestEvent):
            execution_order.append("handler_b")

        @mindflow_event_bus.on(DependsTestEvent, depends_on=Depends(handler_b))
...
async def test_async_handler_with_dependency():
    ...
    with mindflow_event_bus.scoped_handlers():
        ...
        @mindflow_event_bus.on(DependsTestEvent, depends_on=Depends(sync_setup))
...
async def test_mixed_handlers_with_dependencies():
    ...
    with mindflow_event_bus.scoped_handlers():
        ...
        @mindflow_event_bus.on(DependsTestEvent, depends_on=Depends(setup))
...
async def test_circular_dependency_detection():
    ...
    from mindflow.events.handler_graph import CircularDependencyError, build_execution_plan
