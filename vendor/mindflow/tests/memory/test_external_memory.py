...
from mindflow.crew import Crew, Process
...
from mindflow.memory.external.external_memory_item import ExternalMemoryItem
from mindflow.memory.storage.interface import Storage
...
def patch_configure_mem0(mock_mem0_memory):
    with patch(
        "mindflow.memory.external.external_memory.ExternalMemory._configure_mem0",
...
def test_external_memory_reset(external_memory_with_mocked_config):
    with patch(
        "mindflow.memory.external.external_memory.ExternalMemory.reset"
    ) as mock_reset:
...
def test_crew_external_memory_save_with_memory_flag(
    ...
    with patch(
        f"mindflow.memory.external.external_memory.ExternalMemory.{mem_method}"
    ) as mock_method:
