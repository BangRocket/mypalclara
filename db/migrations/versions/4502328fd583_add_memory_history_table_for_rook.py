...
def upgrade() -> None:
    """Upgrade schema."""
    # Note: intentions, memory_dynamics, memory_supersessions, and memory_access_log
    # are already created in migration a1b2c3d4e5f6. This migration was auto-generated
    # with duplicates. We make it idempotent to handle both fresh installs and upgrades.

    # Create intentions (if not already created by previous migration)
    ...
    # Create memory_dynamics (if not already created by previous migration)
    ...
    # Create memory_history (the main table this migration adds)
    if not table_exists('memory_history'):
    ...
    if table_exists('memory_history'):
        if not index_exists('memory_history', 'ix_memory_history_memory_id'):
            op.create_index(op.f('ix_memory_history_memory_id'), 'memory_history', ['memory_id'], unique=False)
        if not index_exists('memory_history', 'ix_memory_history_memory_id_created'):
            op.create_index('ix_memory_history_memory_id_created', 'memory_history', ['memory_id', 'created_at'], unique=False)

    # Create memory_supersessions (if not already created by previous migration)
    ...
    # Create memory_access_log (if not already created by previous migration)
...
def downgrade() -> None:
    ...
    # Drop memory_history
    if table_exists('memory_history'):
        if index_exists('memory_history', 'ix_memory_history_memory_id_created'):
            op.drop_index('ix_memory_history_memory_id_created', table_name='memory_history')
        if index_exists('memory_history', 'ix_memory_history_memory_id'):
