"""Utility functions for Clara Memory System."""
...
def format_entities(entities):
    """Format entity relationships for display."""
    if not entities:
        return ""
...
def remove_code_blocks(content: str) -> str:
    """Remove enclosing code block markers ```[language] and ``` from a string.

    Also removes <think>...</think> tags (used by some models).
    """
    pattern = r"^```[a-zA-Z0-9]*\n([\s\S]*?)\n```$"
...
def extract_json(text):
    ...
    Removes enclosing triple backticks and optional 'json' tag if present.
...
def get_image_description(image_obj, llm, vision_details):
    """Get the description of the image using LLM."""
...
def process_telemetry_filters(filters):
    """Process the telemetry filters (for backward compatibility with mem0 telemetry)."""
    if filters is None:
        return {}, {}
