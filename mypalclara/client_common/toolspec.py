...
Vendored from `mypalclara.tools._base`, minus the engine-only `to_schema()` /
`ToolSchema` reference, so the client carries no engine import. The client uses
these to *define* CLI tools; tool execution/schema conversion is engine-side.
"""
