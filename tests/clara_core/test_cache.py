...
class TestDualWriteVectorStore:
    """Tests for DualWriteVectorStore."""
    ...
    def test_primary_only_writes_to_primary(self):
        ...
        dual = DualWriteVectorStore(
            mock_primary,
            mock_secondary,
            mode=DualWriteMode.PRIMARY_ONLY,
        )

        dual.insert([[0.1, 0.2]], [{"test": "data"}], ["id-1"])

        mock_primary.insert.assert_called_once()
        mock_secondary.insert.assert_not_called()
    ...
    def test_dual_write_writes_to_both(self):
        ...
        mock_primary.insert.assert_called_once()
        mock_secondary.insert.assert_called_once()
    ...
    def test_secondary_only_writes_to_secondary(self):
        ...
        dual = DualWriteVectorStore(
            mock_primary,
            mock_secondary,
            mode=DualWriteMode.SECONDARY_ONLY,
        )

        dual.insert([[0.1, 0.2]], [{"test": "data"}], ["id-1"])

        mock_primary.insert.assert_not_called()
        mock_secondary.insert.assert_called_once()
    ...
    def test_primary_only_reads_from_primary(self):
        ...
        mock_primary = MagicMock()
        mock_primary.search.return_value = [{"id": "1", "score": 0.9}]
        mock_secondary = MagicMock()
        ...
        result = dual.search("query", [0.1, 0.2], limit=5)

        assert result == [{"id": "1", "score": 0.9}]
        mock_primary.search.assert_called_once()
        mock_secondary.search.assert_not_called()
    ...
    def test_secondary_only_reads_from_secondary(self):
        ...
        mock_primary = MagicMock()
        mock_secondary = MagicMock()
        mock_secondary.search.return_value = [{"id": "2", "score": 0.8}]
        ...
        mock_secondary.search.assert_called_once()
        mock_primary.search.assert_not_called()
    ...
    def test_dual_read_reads_from_both(self):
        ...
        mock_primary.search.return_value = [{"id": "1", "score": 0.9}]
        mock_secondary = MagicMock()
        mock_secondary.search.return_value = [{"id": "1", "score": 0.85}]

        dual = DualWriteVectorStore(
            mock_primary,
            mock_secondary,
            mode=DualWriteMode.DUAL_READ,
        )
    ...
    def test_secondary_failure_does_not_fail_operation(self):
        ...
        mock_primary = MagicMock()
        mock_secondary = MagicMock()
        mock_secondary.insert.side_effect = Exception("Secondary failed")
    ...
    def test_mode_change(self):
        ...
        assert dual.mode == DualWriteMode.PRIMARY_ONLY

        dual.mode = DualWriteMode.DUAL_WRITE
        assert dual.mode == DualWriteMode.DUAL_WRITE

        dual.mode = "secondary_only"  # String should work too
        assert dual.mode == DualWriteMode.SECONDARY_ONLY
    ...
    def test_stats(self):
        ...
        assert stats["mode"] == "dual_write"
        assert stats["comparison_mismatches"] == 0
